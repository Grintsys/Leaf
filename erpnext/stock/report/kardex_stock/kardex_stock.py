...
def execute(filters=None):
	...
	products = []	# for type in type_transactions:
	...
	for product in products:
		...
		for sle in sl_entries:
			if sle.item_code == product:
				...
				if cont_first:
					initial_stock = sle.qty_after_transaction - sle.actual_qty
					cont_first = False

				item_name = sle.item_name
				final_stock = sle.qty_after_transaction

				if sle.actual_qty >= 0:
					income_qty = income_qty + sle.actual_qty					
				else:
					expenses_qty = expenses_qty + sle.actual_qty									
		
		req_data = [product, item_name, initial_stock, income_qty, expenses_qty, final_stock]
...
def get_columns():
	columns = [
		...
		{"label": _("Initial Balance Qty"), "fieldname": "initial_balance_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
		...
		{"label": _("Final Balance Qty"), "fieldname": "final_balance_qty", "fieldtype": "Float", "width": 100, "convertible": "qty"},
	]
