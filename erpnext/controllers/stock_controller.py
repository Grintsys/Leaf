...
class StockController(AccountsController):
	...
	def get_gl_entries_receipt(self, items):
		gl_list = []
		company = frappe.get_doc("Company", self.company)
		warehouse_account = get_warehouse_account_map(self.company)

		if self.set_warehouse == None:
			frappe.throw(_("Need a Accepted Warehouse"))

		amount = 0

		for item_row in items:
			it = frappe.get_doc("Item", item_row.item_code)
			amount = amount + flt(item_row.amount, 2)
			if it.purchase_default_values != None:
				gl_list.append(self.get_gl_dict({
                    "account": it.purchase_default_values,
					"against": it.purchase_default_values,
					"cost_center": self.cost_center,
					"remarks": self.get("remarks") or "Accounting Entry for Stock",
					"debit": flt(item_row.amount, 2),
					"is_opening": it.get("is_opening") or self.get("is_opening") or "No",
                }, warehouse_account[self.set_warehouse]["account_currency"], item=it))
			else:
				gl_list.append(self.get_gl_dict({
					"account": warehouse_account[self.set_warehouse]["account"],
			...
			# if it.inventory_default_values != None:
			# # to target warehouse / expense account
			# 	gl_list.append(self.get_gl_dict({
			# 		"account": it.inventory_default_values,
			# 		"against": warehouse_account[self.set_warehouse]["account"],
			# 		"cost_center": self.cost_center,
			# 		"remarks": self.get("remarks") or "Accounting Entry for Stock",
			# 		"credit": flt(item_row.amount, 2),
			# 		"project": it.get("project") or self.get("project"),
			# 		"is_opening": it.get("is_opening") or self.get("is_opening") or "No"
			# 	}, item=it))
			# else:
			# 	gl_list.append(self.get_gl_dict({
		...
		gl_list.append(self.get_gl_dict({
			"account": company.stock_received_but_not_billed,
			"against": warehouse_account[self.set_warehouse]["account"],
			"cost_center": self.cost_center,
			"remarks": self.get("remarks") or "Accounting Entry for Stock",
			"credit": flt(amount, 2),
	...
	def get_gl_entries(self, warehouse_account=None, default_expense_account=None,
		...
		if self.doctype == "Purchase Receipt":
			gl_list = self.get_gl_entries_receipt(self.items)
		else:
	...
	def get_voucher_details(self, default_expense_account, default_cost_center, sle_map):
		if self.doctype == "Stock Reconciliation":
			reconciliation_purpose = frappe.db.get_value(
				self.doctype, self.name, "purpose")
	...
	def get_items_and_warehouses(self):
		...
		if hasattr(self, "items"):
			...
			for row in data[data.index(self.head_row)+1:]:
				d = frappe._dict(
					zip(["item_code", "warehouse", "qty", "valuation_rate"], row))
	...
	def check_expense_account(self, item):
		if not item.get("expense_account"):
			frappe.throw(
				_("Expense or Difference account is mandatory for Item {0} as it impacts overall stock value").format(item.item_code))
       ...
       frappe.db.get_value("Item", d.item_code, inspection_required_fieldname)):
			...
			if self.docstatus == 1 and d.quality_inspection:
				...
				if qa_doc.docstatus == 0:
					...
					frappe.throw(_("Quality Inspection: {0} is not submitted for the item: {1} in row {2}").format(
						link, d.item_code, d.idx), QualityInspectionNotSubmittedError)
				...
				if self.docstatus ==1 and action == 'Stop':
                  ...
                  exc=QualityInspectionRequiredError)
					...
					frappe.msgprint(_("Create Quality Inspection for Item {0}").format(
						frappe.bold(d.item_code)))

	def update_blanket_order(self):
		blanket_orders = list(
			set([d.blanket_order for d in self.items if d.blanket_order]))
...
def update_gl_entries_after(posting_date, posting_time, for_warehouses=None, for_items=None,
	...
	future_stock_vouchers = get_future_stock_vouchers(
		posting_date, posting_time, for_warehouses, for_items)
					...
					if expected_gle:
						if not existing_gle or not compare_existing_and_expected_gle(existing_gle, expected_gle):
							...
							voucher_obj.make_gl_entries(
								gl_entries=expected_gle, repost_future_gle=False, from_repost=True)
					else:
						_delete_gl_entries(voucher_type, voucher_no)
